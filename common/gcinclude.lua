...
-- Efficiency improvement: AliasList as a set for O(1) lookup
gcinclude.AliasList = {
    'aeonic','ammo','autodw','autoDW','autohasso','autohaste','automa','autoMA','brdbonus','bullet','burst','cj','com','compression','cormsg','craftset','curecheat','death','det','detonation','dis','distortion','dncparty','dt','element','empyrean','externalphalanx','fight','fishset','forcestring','frag','fragmentation','fra','fusion','fus','gcaspir','gcdrain','gcmessages','geobonus','gra','grav','gravitation','gun','helix','hpset','imp','impaction','ind','induration','killingblow','kite','liquefaction','liq','meleeset','MH','mh','mwep','mythic','nuke','nukeset','oh','OH','phalanxset','plap','proc','pupmode','reverberation','rev','rwep','rw','rrset','scission','sci','setcycle','shield','siphon','sir','skillchains','string','sub','Sub','tankset','telering','th','ullr','vagarysc','warpring','weapon','weaponlock','weather','wsdistance','zeniset','zz','proc1h','proc2h',
}
local AliasSet = {}
...
function gcinclude.SetVariables()
    ...
    gcdisplay.CreateToggle('DTset', false, 1000)
    gcdisplay.CreateToggle('Kite', false, 999)
    gcdisplay.CreateToggle('TH', false, 998)

    -- AutoDW jobs
    local autoDWMainJobs = { NIN=true, DNC=true, BLU=true, THF=true }
    local autoDWSubJobs = { NIN=true, DNC=true }
    if autoDWMainJobs[mainJob] or autoDWSubJobs[subJob] then
        gcdisplay.CreateToggle('AutoDW', true, 1)
    end

    gcdisplay.CreateCycle('MeleeSet', { 'Default', 'Hybrid', 'Acc' }, 50)

	-- AutoMA jobs
	local autoMAJobs = { MNK = true, PUP = true }
	if autoMAJobs[mainJob] then
		gcdisplay.CreateToggle('AutoMA', true, 1)
	end
    ...
    -- PLD setup
    if mainJob == 'PLD' then
        gcdisplay.CreateToggle('SIR', true, 1)
        gcdisplay.CreateCycle('TankSet', { 'Main', 'MEVA', 'None' }, 3)
        gcdisplay.CreateCycle('OH', { 'Duban', 'Aegis', 'Blurred' }, 2)
        gcdisplay.CreateCycle('MH', { 'Burtgang', 'Naegling' }, 1)
    end
    ...
    -- SAM setup
    if mainJob == 'SAM' then
        gcdisplay.CreateToggle('Proc', false, 2)
        gcdisplay.CreateToggle('AutoHasso', true, 1)
        gcdisplay.CreateCycle('MWep', { 'Doji', 'Pole' }, 1)
    end
    ...
    -- COR setup
    if mainJob == 'COR' then
        gcdisplay.CreateCycle('MH', { 'Naegling', 'Melee Rostam', 'Range Rostam' }, 1)
        gcdisplay.CreateCycle('RWep', { 'Death Penalty', 'Fomalhaut.', 'TP Bonus' }, 3)
        gcdisplay.CreateCycle('Ammo', { 'MAB', 'Phys.', 'Brz.' }, 4)
        if subJob == 'DNC' or subJob == 'NIN' then
            gcdisplay.CreateCycle('OH', { 'Gleti', 'Degen', 'Tauret', 'Roll Rostam' }, 2)
        end
    end
    ...
    if mainJob == 'BLU' then
        ...
        gcdisplay.CreateCycle('MH', { 'Tizona', 'Naegling', 'Maxentius' }, 1)
        gcdisplay.CreateCycle('OH', { 'TP Bonus', 'Sakpata', 'Bunzi' }, 2)
    end

    -- RDM setup
    if mainJob == 'RDM' then
        gcdisplay.CreateCycle('MH', { 'Crocea', 'Naegling', 'Maxentius', 'Tauret' }, 1)
        gcdisplay.CreateToggle('Ullr', false, 20)
        if subJob == 'DNC' or subJob == 'NIN' then
            gcdisplay.CreateCycle('OH', { 'Degen', 'TP Bonus', 'Bunzi' }, 2)
        end
    end

    -- NIN setup
    if mainJob == 'NIN' then
        gcdisplay.CreateCycle('MH', { 'Heishi', 'Naegling', 'Tauret', }, 1)
        gcdisplay.CreateToggle('Proc', false, 200)
        gcdisplay.CreateCycle('Proc1H', { 'Off', 'Sword', 'Dagger', 'Club', 'Katana', }, 20)
        gcdisplay.CreateCycle('Proc2H', { 'Off', 'GKT', 'GS', 'Staff', 'Scythe', 'Pole' }, 21)
        gcdisplay.CreateCycle('OH', { 'Tauret', 'Naegling', 'Gleti', 'Kunimitsu', }, 2)
        gcdisplay.CreateCycle('Ammo', { 'Shuriken', 'Coiste', 'Lucky Egg', 'Staunch' }, 3)
    end
    ...
    if mainJob == 'WHM' then
        ...
        if subJob == 'DNC' or subJob == 'NIN' then
            gcdisplay.CreateCycle('OH', { 'Degen', 'TP Bonus', 'Bunzi' })
        end
    end]]--
end
--;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
function gcinclude.HandleCommands(args)
    ...
    local player = gData.GetPlayer()
    if not AliasSet[cmd] then return end
