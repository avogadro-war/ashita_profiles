...
function gcinclude.SetVariables()
	...
	if (player.MainJob == 'COR') then
		gcdisplay.CreateCycle('MH', {[1] = 'Naegling', [2] = 'Melee Rostam', [3] = 'Range Rostam'});
		gcdisplay.CreateCycle('RWep', {[1] = 'Death Penalty', [2] = 'Fomalhaut.', [3] = 'TP Bonus'});
		gcdisplay.CreateCycle('Ammo', {[1] = 'MAB', [2] = 'Phys.', [3] = 'Brz.'});
		if (player.SubJob == 'DNC' or player.SubJob == 'NIN') then
			gcdisplay.CreateCycle('OH', {[1] = 'Gleti', [2] = 'Tauret', [3] = 'Roll Rostam'});	
		end
	end
	...
	if (player.MainJob == 'RDM') then
		gcdisplay.CreateCycle('MH', {[1] = 'Crocea', [2] = 'Naegling', [3] = 'Maxentius', [4] = 'Tauret'});
		gcdisplay.CreateToggle('Ullr', false);
		if (player.SubJob == 'DNC' or player.SubJob == 'NIN') then
			gcdisplay.CreateCycle('OH', {[1] = 'Degen', [2] = 'TPBonus', [3] = 'Bunzi'});	
		end
	end
end
...
function gcinclude.HandleCommands(args)
	...
	if player.MainJob == 'RDM' or player.MainJob == 'COR' then
		if string.lower(args[1]) == 'mh' then
			gcdisplay.AdvanceCycle('MH');
			toggle = 'MH';
			status = gcdisplay.GetCycle('MH');
