...
local function updateJobSnapshot()
    local mainJob = gData.GetPlayer().MainJob;
    courserRoll = 0

    local mainJobSnapshot = {
        COR = 102, 
        RNG = 102
    }

    jobSnapshot = mainJobSnapshot[mainJob] or 0;
end
...
updateJobMA();
onMainJobChange:register(updateJobMA);
onSubJobChange:register(updateJobMA);

updateJobSnapshot();
onMainJobChange:register(updateJobSnapshot);
onSubJobChange:register(updateJobSnapshot);
...
onBuffGain:register(function(buffId)
    ...
    elseif (buffId == 228 or buffId == 214 or buffId == 604 or buffId == 580) then
        hasteChange:trigger(true);
    elseif (buffId == 581 or buffId == 265 or buffId == 332) then
        hasteChange:trigger(true);
    end
end);
...
local function getMaSnapshot()
    local maSnapshot = 0;

    if buffactive()[581] then -- Flurry 
        maSnapshot = maSnapshot + (flurryLevel == 2 and 307 or 150);
    end
...
local function getTotalSnapshot()
    local maSnapshot = getMaHaste();
    local embravaSnapshot = buffactive()[228] and 266 or 0;

    local total = maSnapshot + embravaSnapshot;
    return total <= 819 and total or 819;
end
...
local function GetDWGearSet(pool)
    ...
    if type(dwNeeded) ~= 'number' or dwNeeded <= 0 or type(pool) ~= 'table' then return {}, 0 end

    local bestSet = {}
    local bestTotalDW = -1

    local n = #pool
    local totalCombos = 2^n

    for i = 1, totalCombos - 1 do
        local currentSet = {}
        local sumDW = 0
        ...
        for j = 1, n do
            if bit.band(i, bit.lshift(1, j - 1)) ~= 0 then
                ...
                local dwVal = gear[4]
                sumDW = sumDW + dwVal
                currentSet[slot] = item
            end
        end

        if sumDW <= dwNeeded and sumDW > bestTotalDW then
            bestSet = currentSet
            bestTotalDW = sumDW
        end
    end

    return bestSet, bestTotalDW
end
...
--[[if player.MainJob == 'COR' or player.MainJob == 'RNG' then
    ...
    packetsIncoming[0x28] = function(e)
        
        local playerId = AshitaCore:GetMemoryManager():GetParty():GetMemberServerId(0)
        local bit_offset = 32 -- after actor ID
        local target_count = ashita.bits.unpack_be(e.data_raw, bit_offset, 8)
        bit_offset = bit_offset + 8

        for t = 1, target_count do
            local targetId = ashita.bits.unpack_be(e.data_raw, bit_offset, 32)
            bit_offset = bit_offset + 32

            local action_count = ashita.bits.unpack_be(e.data_raw, bit_offset, 4)
            bit_offset = bit_offset + 4

            for a = 1, action_count do
                -- skip reaction + animation
                bit_offset = bit_offset + 5 + 11

                local category = ashita.bits.unpack_be(e.data_raw, bit_offset, 4)
                bit_offset = bit_offset + 4

                local roll_id = ashita.bits.unpack_be(e.data_raw, bit_offset, 10)
                bit_offset = bit_offset + 10

                -- skip message + unknown
                bit_offset = bit_offset + 10 + 31

                local rollNumber = ashita.bits.unpack_be(e.data_raw, bit_offset, 17)
                bit_offset = bit_offset + 17

                if targetId == playerId and category == 6 and roll_id == 120 then
                    courserRoll = rollNumber
                end
            end
        end
        ...
        if (category == 4) then
            ...
            if (isTarget(action)) then
                ...
                elseif (param == 511 or param == 710) then
                    hasteLevel = 2;
                    hasteChange:trigger()
                elseif (param == 845 and flurryLevel ~= 2) then --Flurry I
                    flurryLevel = 1;
                    hasteChange:trigger();
                elseif (param == 846) then -- Flurry II
                    flurryLevel = 2;
                    hasteChange:trigger()
